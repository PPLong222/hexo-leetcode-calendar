...
width = leetCanvas.width = getComputedStyle(leetCanvas).width.match(/[0-9]+/)[0];
...
let userName = document.getElementsByClassName('datasource')[0].getElementsByTagName('a')[0].innerHTML.substring(1)
...
//       keys = Object.keys(_rawData)
//       values = Object.values(_rawData)

//       for(i in keys) {
//         // 因为Leetcode返回的数据是以秒为单位的，这里做一个统一
//         map.set(keys[i]*1000, values[i])
//       }
//       handle(map);
//     }
// );
...
function handle(data) {
    ...
    height = leetCanvas.height = calendarTopMargin + monthFontSize + 7 * (divSize + divMargin) + calendarBottomMargin
    ...
    for (col = 0; col < maxColSize; col++) {
        ...
        for (row = 0; row < tempRow; row++) {
            ...
            if (data.get(curStamp) != null) {
                leetCtx.fillStyle = subColors[Math.min(data.get(curStamp) / submitGap + 1, subColors.length - 1)]
            } else {
                leetCtx.fillStyle = subColors[0]
            }
    ...
    function generateDataSource() {
        // 更新数据来源左边margin
        document.getElementsByClassName("datasource")[0].style = "margin-left: " + calendarLeftMargin + "px"
    }
